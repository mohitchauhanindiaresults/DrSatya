...
class _AddEnquiryScreenState extends State<AddEnquiryScreen> {
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    if (centerController.text.isEmpty ||
                      ...
                      String? response =
                          await Utils.getStringFromPrefs(Constant.MEMBER_API);

                      if (isMobileNumberFound(
                          response!, mobileController.text)) {
                        Utils.showAlertDialog(context, "Number already found!");
                      } else {
                        if (isMobileNumberFound(
                            response!, alternativeMobileController.text)) {
                          Utils.showAlertDialog(
                              context, "Alternative number already found!");
                        } else {
                          addEnquiry(context);
                        }
                      }
  ...
  Future<void> addEnquiry(BuildContext context) async {
    ...
    try {
      final response =
      await dio.post(Constant.BASE_URL + "api/addenquiry", data: data);
